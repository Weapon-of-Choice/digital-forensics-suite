    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {cases?.map(c => (
            <div key={c.id} className="bg-white border border-slate-200 rounded-lg p-5 hover:border-violet-300 hover:shadow-md transition group">
              <div className="flex items-start justify-between">
                <Link to={`/cases/${c.id}`} className="flex-1">
                  ...
                  <p className="text-sm text-slate-500 mt-1">{c.description || 'No description'}</p>
